...
use crate::{
    models::{Article, ArticleRef, Collection},
    services::data_processor::html_to_markdown,
};
use log::{error, info};
...
impl DataProcessor {
    pub async fn prepare_sync_collection(
        &self,
        collection: &Collection,
    ) -> Result<(), anyhow::Error> {
        info!(
            "Preparing to sync collection: ID:{:?}, Slug: {:?}",
    ...
    pub async fn sync_collection(&self, collection: &Collection) -> Result<()> {
        info!(
            "Storing collection: ID:{:?}, Slug: {:?}",
            collection.id, collection.slug
        );
        let mut conn = self.db_pool.get().context("Failed to get DB connection")?;

        collection.store(&mut conn).with_context(|| {
            format!(
                "Failed to store collection: ID:{:?}, Slug:{:?}",
                collection.id, collection.slug
            )
        })?;

        info!(
            "Successfully stored collection: ID:{:?}, Slug:{:?}",
            collection.id, collection.slug
        );
        Ok(())
    }
    ...
    pub async fn sync_article(
        ...
        if let Err(e) = self.convert_html_to_markdown(&article).await {
            error!(
                "Failed to convert HTML to Markdown for article ID:{}: {}",
                article.id, e
            );
    ...
    pub async fn store_article(&self, article: &Article) -> Result<Article> {
        ...
        let mut conn = self.db_pool.get().context("Failed to get DB connection")?;
        let article = article.store(&mut conn)?;
        Ok(article)
    }

    pub async fn convert_html_to_markdown(&self, article: &Article) -> Result<()> {
        info!(
            "Converting HTML to Markdown for article: ID:{:?}, Title: {:?}",
            article.id, article.title
        );
        let markdown = html_to_markdown(article.html_content.as_ref().ok_or_else(|| {
            anyhow::anyhow!("HTML content not found for article ID:{}", article.id)
        })?)?;
        ...
        article
            ...
            .context(format!(
                "Failed to update markdown content for article ID:{}",
                article.id
            ))?;
