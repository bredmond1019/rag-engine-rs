...
impl SearchService {
    ...
    pub async fn collection_based_search(
        ...
        let relevant_collections =
            Collection::find_relevant_collections(&query_embedding.clone().into(), &mut conn)?;

        // Get articles from relevant collections
        let mut article_ids: Vec<Uuid> = Vec::new();
        for (collection, _) in relevant_collections {
            let collection_article_ids =
                Article::article_ids_by_collection(&collection, &mut conn)?;
            article_ids.extend(collection_article_ids);
        }

        // Perform two-stage retrieval on the subset of articles

        let semantic_results = Article::find_relevant_articles_by_ids(
